...
package com.googlesource.gerrit.plugins.changehead;
...
public final class ChangeHeadCommand extends SshCommand {
    ...
    protected void run() throws UnloggedFailure, Failure, Exception {
      ...
      try {
        repo = repoManager.openRepository(project.getProject().getNameKey());

        if (repo == null) {
          throw new UnloggedFailure(1, "Unable to find project.");
        }

        if (!project.isOwner()) {
          throw new UnloggedFailure(1, "Permission denied: you do not have ownership rights to " +
              project.getProject().getName());
        }

        final Ref ref = repo.getRef(newHeadBranch);
        if (ref == null) {
          throw new UnloggedFailure(1, "Destination branch " + " does not exist.");
        }

        final RefUpdate u = repo.updateRef(Constants.HEAD, true);
        u.disableRefLog();
        RefUpdate.Result res = u.link(ref.getName());
        switch(res) {
          case NO_CHANGE:
          case RENAMED:
            break;
          case IO_FAILURE:
          case LOCK_FAILURE:
            throw new UnloggedFailure(1, "Unable to set HEAD, try again later.");
        }
